...
-- ---------- 4) CR-M2-001 权限矩阵补齐（2026-09-21 CCB 会签通过 · PM 签发） ----------
-- 真源：docs/change/CR-M2-001-permission-matrix-gap.md（PRD v1.1 §2.2 新增 5 行矩阵）
-- 变更量：权限点 16 → 22（+6，id 2017–2022）｜授权 23 → 40（+17，id 3024–3040）
--         敏感点 3 → 4（新增 data:reveal，须二次验证 + 留痕）
-- 取舍：elder:archive:read / rule:view 五角色全授予，可见范围由 data_scope 控制（不分裂权限点）。
INSERT IGNORE INTO `sys_permission` (`id`, `perm_code`, `perm_name`, `module`, `need_second_verify`, `deleted`) VALUES
  -- 新增矩阵行 10 老人建档
...
-- 新增授权（id 3024–3040，共 17 条）：角色 1001 ELDER / 1002 ASSESSOR / 1003 FAMILY / 1004 ORG_ADMIN / 1005 SUPERVISOR
INSERT IGNORE INTO `sys_role_permission` (`id`, `role_id`, `perm_id`) VALUES
