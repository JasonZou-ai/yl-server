...
> 编号 **YL-M2-ER14-BINDING**｜起草 2026-09-21｜状态：**提案，待 ER 三方面裁决**
...
| 议题 | 建议口径 | 依据 |
|---|---|---|
| **留存期** | **随账号存续**；解绑 → `deleted=1`；**账号注销 → 物理删除**（不适用 180 天埋点规则） | ER-03 分档未覆盖此表；绑定关系属「实现合同目的所必需」，账号灭失即失必要性（PIPL 第 19/47 条） |
...
| **测试回归** | 断言 43 → **49 项**（新增 ER-14 断言 6 条）；`verify-schema.sh` 需重建库后全量重跑，既有 43 项口径不变。 |
...
| 4 | `active_uk` 生成列（全库 **3 → 4**）；`deleted` 列（19 → 20） | 3→4 / 19→20 |
...
| # | 裁决点 | 建议 |
|---|---|---|
| 1 | 是否本期建表（还是并入 V2） | **本期建**：渠道登录是 P0 且已实现，缺表会使「免手机号登录」永久不可达 |
| 2 | `openid` 是否**允许留存明文**（便于排障/对账） | **不留明文**，仅 HMAC 摘要 —— 与既实现承诺一致 |
| 3 | 双唯一键口径是否接受（同用户同平台唯一 + 同平台同 openid 唯一） | 接受 |
| 4 | 留存期：随账号存续 + 注销物理删除 | 接受（若要求「解绑后保留 N 年以备监管」，需改用 `retain_until` 模式） |
| 5 | 是否要求「换绑需二次验证」（账号被冒用风险） | **建议要求**：换绑/解绑纳入敏感操作（`need_second_verify=1`），与 `account:family:unbind` 同口径 |

---

## 六、生效条件与落地清单

**生效条件**：ER 三方面（产品 / 研发 / 合规）对 §五 五项裁决达成一致。

| # | 动作 | 产出 |
|---|---|---|
| 1 | 应用 DDL 补丁 | `02_schema.sql` 追加建表（补丁见 `ER-14-DDL-Patch-Proposal.sql`） |
| 2 | 补断言 | `verify-schema.sh` +6 项（43 → 49）；表数断言 37 → 38 |
| 3 | 代码 | `SysUserThirdParty` 实体 + Mapper；`ThirdPartyBindingResolver` 增「按摘要查绑定表」实现 |
| 4 | 适配器降级链 | `rpW1xZ`：**先查绑定 → 未命中再取手机号 → 仍无则报未绑定**（不伪装成「用户未绑定」） |
| 5 | 文档同步 | 设计 §6.2 缺口关闭；README 表数 37；ER 纪要 Action Item 增 ER-14 行 |
| 6 | 合规 | 若裁决「换绑需二次验证」，则 `03_seed_rbac.sql` 增敏感点（并入 CR-M2-001 或另立） |
