...
> 本变更单由研发侧起草，**会签栏留空**——须由对应角色实际签署后方可生效，研发侧不得代签。
...
| # | 动作 | 产出 |
|---|---|---|
| 1 | 应用种子补丁 | `docker/mysql/init/03_seed_rbac.sql` 追加 6 权限点 + 17 授权（id 2017–2022 / 3001+） |
| 2 | 同步代码常量 | `PermissionCode` 增 6 常量；`SensitivePermissions` 增 `data:reveal` |
| 3 | 增补断言 | `verify-schema.sh`：权限点 = 22、授权 = 40、敏感点 = 4、新增 ❌ 红线 8 条（43 → 51 项） |
